...
├── .apm/                       # APM primitives (source of truth)
│   ├── instructions/           # Scoped coding standards
│   ├── agents/                 # AI assistant personas
...
├── AGENTS.md                   # AI guidelines (generated by apm compile)
...
This repo uses [APM (Agent Package Manager)](https://github.com/microsoft/apm) to manage AI agent configuration. All agent instructions, prompts, and skills are declared in `.apm/` and compiled into `AGENTS.md` files.

### Quick Setup (APM)

```bash
pip install apm-cli              # Install APM CLI
apm install && apm compile       # Install dependencies & compile AGENTS.md
```
...
| Directory | Purpose |
|-----------|---------|
| `.apm/instructions/` | Scoped coding standards (`.instructions.md`) |
| `.apm/agents/` | AI assistant personas (`.agent.md`) |
| `.apm/prompts/` | Reusable workflows (`.prompt.md`) |
| `.apm/context/` | Project knowledge (`.context.md`) |
| `.github/skills/` | Copilot skills (`SKILL.md`) |
...
See [AGENTS.md](AGENTS.md) for compiled guidelines.
